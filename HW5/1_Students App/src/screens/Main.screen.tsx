...
const Main = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const maxAbsents = params.get("max-absents");
    const minAbesnts = params.get("min-absents");
    console.log(minAbesnts,maxAbsents);
    ...
    if (maxAbsents) {
      setFilteredList(oldState => (oldState.filter(std => std.absents <= Number(maxAbsents))));
      }
      if (minAbesnts) {
        setFilteredList(oldState => (oldState.filter(std => std.absents >= Number(minAbesnts))));
          }
  ...
  const handleAbsentsfilter=(event: React.ChangeEvent<HTMLInputElement>)=>{
    const inputName: string = event.target.name;
    const value: string =event.target.value;
    
    if(inputName==="Max-abs"){
      const param = params.get("max-absents");
      
      if(Number(value)===0){
        params.delete('max-absents');
        params.delete("min-absents");
      }else if(Number(value)>0){
        if(param){
          params.set("max-absents",value);
         if(params.get("min-absents")===params.get("max-absents")){
          params.set("min-absents",String(Number(params.get("max-absents"))-1));
         }
        }else{
           params.append("max-absents",value);
        }
      }
    }else if(inputName==="Min-abs"){
      const param = params.get("min-absents");

      if(Number(value)===0){
        params.delete('min-absents');
      }else if(Number(value)>0){
        if(param){
          params.set("min-absents",value);
        }else{
           params.append("min-absents",value);
        }
      }
    }
    setParams(params);
  ...
  return (
    <div className="main-screen">
      ...
      <div className="filter">
        ...
        <div className="input">
        <label htmlFor="Min-abs">Min Absents </label>
        <input
          id="Min-abs"
          type="number"
          name="Min-abs"
          min={0}
          max={Number(params.get("max-absents"))-1}
          value={Number(params.get("min-absents"))||0}
          onChange={handleAbsentsfilter}
        />
      </div>
      <div className="input">
        <label htmlFor="Max-abs">Max Absents </label>
        <input
          id="Max-abs"
          name="Max-abs"
          type="number"
          min={0}
          max={30}
          value={Number(params.get("max-absents"))||0}
          onChange={handleAbsentsfilter}
        />
      </div>
      </div>
